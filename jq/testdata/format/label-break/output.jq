...
| .variants[] as $variant
| ( # "oraclelinux9", "slim-trixie", "windows/windowsservercore-ltsc2025"
	$variant | split("/")[-1]
) as $variantName
| [
	$variantName, # "windowsservercore-ltsc2025", etc
	if $variant == $latestOracle then "oracle" else empty end,
	if $variant == $latestSlim then "slim" else empty end,
	empty
...
| [
	...
	if
		$variant == $latestOracle or ($variant | startswith("windows/windowsservercore-"))
	then
		""
	else empty end,
	empty
