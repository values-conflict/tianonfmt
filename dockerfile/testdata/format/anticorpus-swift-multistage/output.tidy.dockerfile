...
RUN set -eux; \
	...
	apt-get -q update; \
	apt-get -q install -y     binutils     curl     git     unzip     gnupg2     libc6-dev     libcurl4-openssl-dev     libncurses5-dev     libedit2     libgcc-13-dev     libpython3-dev     libsqlite3-0     libstdc++-13-dev     libxml2-dev     libz3-dev     pkg-config     tzdata     zlib1g-dev; \
...
RUN set -eux; \
	set -e;     export $(curl -s ${PLATFORM_WEBROOT}/latest-build.yml | grep 'download:' | sed 's/:[^:\/\/]/=/g'); \
	export $(curl -s ${PLATFORM_WEBROOT}/latest-build.yml | grep 'download_signature:' | sed 's/:[^:\/\/]/=/g'); \
	export DOWNLOAD_DIR=$(echo $download | sed "s/-${OS_VER}.tar.gz//g"); \
	echo $DOWNLOAD_DIR > .swift_tag; \
	export GNUPGHOME="$(mktemp -d)"; \
	curl -fsSL ${PLATFORM_WEBROOT}/${DOWNLOAD_DIR}/${download} -o latest_toolchain.tar.gz         ${PLATFORM_WEBROOT}/${DOWNLOAD_DIR}/${download_signature} -o latest_toolchain.tar.gz.sig; \
