...
if [ "$#" -eq 0 ]; then
	versions="$(jq --raw-output 'keys | map(@sh) | join(" ")' versions.json)"
	eval "set -- $versions"
fi
...
for version; do
	...
	phpVersions="$(jq --raw-output '.[env.version].phpVersions | map(@sh) | join(" ")' versions.json)"
	eval "phpVersions=( $phpVersions )"
	# get this Joomla versions default PHP version
	defaultPhpVersion="$(jq --raw-output '.[env.version].php' versions.json)"
	# get this Joomla versions variants
	variants="$(jq --raw-output '.[env.version].variants | map(@sh) | join(" ")' versions.json)"
	eval "variants=( $variants )"
	# get this Joomla versions default variant
	defaultVariant="$(jq --raw-output '.[env.version].variant' versions.json)"
	# get the Joomla versions full version eg: 4.0.0
	fullVersion="$(jq --raw-output '.[env.version].version' versions.json)"
	...
	aliases="$(jq --raw-output '.[env.version].aliases | map(@sh) | join(" ")' versions.json)"
